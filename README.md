...
```powershell
uv sync                  # 가상환경 생성 + 의존성 설치 (uv.lock 기준)
uv run maycomb serve     # http://127.0.0.1:8000
```
...
`uv sync`는 dev 의존성(pytest 등)을 기본으로 함께 설치합니다.

```powershell
uv run pytest                        # 43 tests
uv run python scripts\selftest.py    # 실서버 종단간 (serve 후 실행)
uv run python scripts\demo_client.py # 수동 데모 (콘솔에서 직접 응답)
```
